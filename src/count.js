...
function designBody() {
	...
	var noticePeriodEnd = new Date("January 10, 2024");
	noticePeriodEnd.setDate(noticePeriodEnd.getDate() + 90);
	var today = new Date();

	if (today < noticePeriodEnd) {
		const diffInMills = noticePeriodEnd - today;
		const diffDays = Math.ceil(diffInMills / (1000 * 60 * 60 * 24));
		document.getElementById("tcs_notice_period").innerHTML +=
			" (On notice period, " + diffDays + " days remaining)";
	}
}
